...
python3 $HERMES_HOME/skills/devops/autonomous-crew-integration/scripts/task-dispatcher.py &
DISPATCHER_PID=$!

# 2. Start one poller PER PROJECT (background)
for proj in mnemosyne autopilot aires agora edgewalker; do
    python3 $HERMES_HOME/skills/devops/autonomous-crew-integration/scripts/task-poller.py \
        ${proj}-crew \
        $WORKSPACE_ROOT/$proj \
        $proj &
done
