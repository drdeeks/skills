---
title: Self-Healing Scope vs Crew Agent Responsibilities
category: architecture
date: 2026-07-09
failure: >
  Self-healing was at risk of scope creep into fixing project-level issues
  (failing tests, missing API keys, TypeScript errors, build failures) that
  are properly a crew agent's job, not crew infrastructure's job.
root_cause: >
  No explicit boundary had been drawn between "keep the crew alive"
  (infrastructure) and "do the project work" (agents) — both looked like
  "something is broken, fix it" from the self-healing loop's perspective.
resolution: >
  Restricted self-healing-loop.py to five infrastructure-only checks
  (enforcer health, constitution hash integrity, chain integrity, memory
  pipeline promotion, habit violations) and documented that everything
  else routes through the normal dispatcher -> kanban -> poller -> chain
  enforcement path, per direct user correction: "these should all be taken
  care of and fixed by the crews themselves."
prevention: >
  Before adding a new self-healing check, ask: would a human fix this by
  restarting a daemon or checking a hash (infrastructure), or by writing
  code/config (crew agent work)? Only the former belongs in self-healing.
verified: true
---
