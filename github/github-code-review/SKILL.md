...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ${HERMES_DIR:-~/.hermes}/.env ] && grep -q "^GITHUB_TOKEN=" ${HERMES_DIR:-~/.hermes}/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ${HERMES_DIR:-~/.hermes}/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source ${HERMES_DIR:-~/.hermes}/skills/github/github-auth/scripts/gh-env.sh
