...
elif [ -f ${HERMES_DIR:-~/.hermes}/.env ] && grep -q "^GITHUB_TOKEN=" ${HERMES_DIR:-~/.hermes}/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ${HERMES_DIR:-~/.hermes}/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
