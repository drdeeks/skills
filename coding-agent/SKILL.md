...
description: |
  ...
  (spawn in temp dir), (3) refactoring large codebases, (4) iterative coding that
  needs file exploration. NOT for: simple one-liner fixes (just edit), reading code
  (use read tool), thread-bound ACP harness requests in chat (for example spawn/run
...
- Use `pty: true` in bash tool calls
- These agents need a real terminal for input/output

For **Claude Code**, use `--print` mode (no PTY needed):
- Runs in non-interactive mode
- Use `--permission-mode bypassPermissions` for autonomous operation
...
|--------|---------|-------|
| `scripts/main.py` | coding-agent script | Run with python3 |
