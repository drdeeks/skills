...
description: 'Fetch GitHub issues, spawn sub-agents to implement fixes and open PRs,
  then monitor and address PR review comments. Usage: /gh-issues [owner/repo] [--label
  bug] [--limit 5] [--milestone v1.0] [--assignee @me] [--fork user/repo] [--watch]
  [--interval 5] [--reviews-only] [--cron] [--dry-run] [--model glm-5] [--notify-channel
  -1002381931352]'
metadata:
  openclaw:
    primaryEnv: GH_TOKEN
...
```
cat ${OPENCLAW_CONFIG} | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```
...
- If curl returns an HTTP 401 or 403 → stop and tell the user:
  > "GitHub authentication failed. Please check your apiKey in the OpenClaw dashboard or in ${OPENCLAW_CONFIG} under skills.entries.gh-issues."
...
GH_TOKEN=$(cat ${OPENCLAW_CONFIG} 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty') || GH_TOKEN=$(cat /data/.clawdbot/openclaw.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty')
...
export GH_TOKEN=$(cat ${OPENCLAW_CONFIG} 2>/dev/null | node -e "const fs=require('fs');const d=JSON.parse(fs.readFileSync(0,'utf8'));console.log(d.skills?.entries?.['gh-issues']?.apiKey||'')")
...
|--------|---------|-------|
| `scripts/validate.py` | gh-issues script | Run with python3 |
| `scripts/main.py` | gh-issues script | Run with python3 |

## Key References

- **Overview**: [references/overview.md](references/overview.md)
- **GitHub API**: [references/github-api.md](references/github-api.md)
- **Workflow Guide**: [references/workflow-guide.md](references/workflow-guide.md)
