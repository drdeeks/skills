...
- copy OpenClaw skills into `${HERMES_DIR:-~/.hermes}/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Hermes workspace
- mirror compatible workspace assets such as `workspace/tts/` into `${HERMES_DIR:-~/.hermes}/tts/`
...
- `${HERMES_DIR:-~/.hermes}/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py`

Do not guess a shorter path like `${HERMES_DIR:-~/.hermes}/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `${HERMES_DIR:-~/.hermes}/skills/migration/openclaw-migration/`.
...
```json
{"command":"python3 ${HOME}/.hermes/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py","workdir":"/home/USER"}
```
...
5. Prefer the primary OpenClaw workspace (`${OPENCLAW_DIR:-~/.openclaw}/workspace/`) over `workspace.default/`. Only use the default workspace as fallback when the primary files are missing.
...
- OpenClaw skills available under `${HERMES_DIR:-~/.hermes}/skills/openclaw-imports/`
