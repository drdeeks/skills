...
self_heal_service() {
    ...
    if [[ "$service_running" == "false" ]]; then
        ...
        if [[ "$(uname)" == "Darwin" ]]; then
            ...
            if [[ -f "$HOME/Library/LaunchAgents/com.usbcompute.autostart.plist" ]]; then
                run_cmd "launchctl unload '$HOME/Library/LaunchAgents/com.usbcompute.autostart.plist'" "Unloading LaunchAgent"
                run_cmd "launchctl load '$HOME/Library/LaunchAgents/com.usbcompute.autostart.plist'" "Loading LaunchAgent"
            fi
        elif [[ "$(expr substr $(uname -s) 1 5)" == "Linux" ]]; then
...
run_cleanup() {
    ...
    local thumb_caches=("$HOME/.thumbnails" "$HOME/.cache/thumbnails")
...
install_llama_cpp() {
    ...
    if test_path "$INSTALL_PATH_LLAMA/main" "llama.cpp main executable" && \
       test_installation "llama.cpp" "$INSTALL_PATH_LLAMA/main --help" "llama.cpp"; then
        ...
        # Auto-install llmrl alongside llama.cpp — they're a natural pair
        # (browse/download HF GGUFs, then run with llama.cpp). User can opt
        # out by skipping the "install llama.cpp" prompt entirely.
        if command -v npm >/dev/null 2>&1 && [[ -f "$SCRIPT_DIR/setup-llmrl.sh" ]]; then
            print_info "Auto-installing llmrl (HuggingFace model browser) alongside llama.cpp..."
            install_llmrl || print_warning "llmrl auto-install failed — run manually with: bash $SCRIPT_DIR/setup-llmrl.sh"
        fi
        return 0
    else
...
install_llmrl() {
    print_header "Installing llmrl (HuggingFace model browser/downloader)"

    if ! command -v npm >/dev/null 2>&1; then
        print_error "npm not found — install Node.js first (install_nodejs_tools)"
        return 1
    fi

    local boot="$SCRIPT_DIR/setup-llmrl.sh"
    if [[ ! -f "$boot" ]]; then
        print_error "Bootstrapper missing: $boot"
        return 1
    fi

    local target="${INSTALL_PATH_AI_SYSTEMS:-/opt/ai-systems}/llmrl"
    mkdir -p "$(dirname "$target")"
    if run_cmd "cd '$(dirname "$target")' && MODEL_DIR='${MODEL_DIR:-/opt/llm-models}' bash '$boot'" "Bootstrapping llmrl"; then
        if test_installation "llmrl" "llmrl 2>&1 | head -1" "llmrl"; then
            save_installation_record "llmrl" "$target" "1.0.0" "installed"
            return 0
        fi
    fi
    print_error "llmrl installation verification failed"
    return 1
}
...
main() {
    ...
    if print_info "Do you want to install LLM engines?" && \
       confirm "Install LLM engines?"; then
        ...
        if confirm "Install llmrl (HuggingFace model browser/downloader CLI)?"; then
            if install_llmrl; then
                print_success "llmrl installed successfully"
            else
                print_error "llmrl installation failed"
                if ! confirm "Continue despite llmrl failure?" "n"; then
                    exit 1
                fi
            fi
        fi
