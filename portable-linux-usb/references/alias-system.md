...
```bash
cat scripts/usb-aliases.sh >> ${HOME}/.bashrc
source ${HOME}/.bashrc
```
...
# Full USB restore
alias usb-restore='sudo dd if=${HOME}/usb-backup.img of=/dev/sdX bs=4M status=progress'

# Backup persistence only
alias usb-backup-persist='sudo dd if=/dev/sdX5 of=${HOME}/persist-backup-$(date +%Y%m%d).dat bs=4M status=progress'

# Restore persistence only
alias usb-restore-persist='sudo dd if=${HOME}/persist-backup.dat of=/dev/sdX5 bs=4M status=progress'
```
...
usb-custom-alias() {
    ...
    echo "alias $name='$command'" >> ${HOME}/.bashrc
...
Create `${HOME}/.usb-aliases.conf`:

```bash
# ${HOME}/.usb-aliases.conf
...
USB_BACKUP_DIR="${HOME}/usb-backups"
USB_BACKUP_KEEP="5"
```
...
```bash
# Add to ${HOME}/.bashrc:
if [ -f ${HOME}/.usb-aliases.conf ]; then
    source ${HOME}/.usb-aliases.conf
fi
```
...
grep "alias usb-" ${HOME}/.bashrc | sort
```
