...
sudo dd if=/dev/sdX bs=4M status=progress | gzip > ${HOME}/usb-full-$(date +%Y%m%d).img.gz

# Or with progress and hash verification:
sudo dd if=/dev/sdX of=${HOME}/usb-full-$(date +%Y%m%d).img bs=4M status=progress
sha256sum ${HOME}/usb-full-$(date +%Y%m%d).img > ${HOME}/usb-full-$(date +%Y%m%d).img.sha256
```
...
sudo dd if=/dev/sdX5 of=${HOME}/persistence-$(date +%Y%m%d).dat bs=4M status=progress

# Backup all agents:
for agent in alpha beta gamma; do
    sudo dd if=/media/ventoy-usb/agents/${agent}-home.dat \
        of=${HOME}/agent-${agent}-$(date +%Y%m%d).dat bs=4M status=progress
done

# Backup home directory:
sudo tar czf ${HOME}/home-$(date +%Y%m%d).tar.gz -C /media/ventoy-usb persistence/

# Backup Ventoy config:
sudo cp -r /media/ventoy-usb/ventoy ${HOME}/ventoy-config-$(date +%Y%m%d)
```
...
sudo rsync -av --delete \
    --link-dest=${HOME}/usb-backup/latest \
    /media/ventoy-usb/ ${HOME}/usb-backup/$(date +%Y%m%d)/

# Update latest symlink:
ln -sfn ${HOME}/usb-backup/$(date +%Y%m%d) ${HOME}/usb-backup/latest
```
...
gunzip -c ${HOME}/usb-backups/YYYYMMDD-HHMMSS/usb-image.img.gz | \
    sudo dd of=/dev/sdX bs=4M status=progress

# Or decompress first, then restore:
gunzip ${HOME}/usb-backups/YYYYMMDD-HHMMSS/usb-image.img.gz
sudo dd if=${HOME}/usb-backups/YYYYMMDD-HHMMSS/usb-image.img of=/dev/sdX bs=4M status=progress
```
...
sudo dd if=${HOME}/usb-backups/YYYYMMDD-HHMMSS/persistence.dat \
    of=/dev/sdX5 bs=4M status=progress

# Restore specific agent:
sudo dd if=${HOME}/usb-backups/YYYYMMDD-HHMMSS/agent-alpha.dat \
    of=/media/ventoy-usb/agents/alpha-home.dat bs=4M status=progress

# Restore Ventoy config:
sudo cp -r ${HOME}/usb-backups/YYYYMMDD-HHMMSS/ventoy-config/* /media/ventoy-usb/ventoy/
```
...
rclone sync ${HOME}/usb-backups remote:usb-backups

# Or with AWS S3:
aws s3 sync ${HOME}/usb-backups s3://my-usb-backups/

# Or with Google Cloud:
gsutil rsync -r ${HOME}/usb-backups gs://my-usb-backups/
```
...
```bash
# List all backups:
ls -lh ${HOME}/usb-backups/

# List by date:
ls -lt ${HOME}/usb-backups/ | head -10

# Show backup details:
cat ${HOME}/usb-backups/YYYYMMDD-HHMMSS/manifest.json
```
...
sha256sum -c ${HOME}/usb-backups/YYYYMMDD-HHMMSS/usb-image.img.gz.sha256

# Test restore to temporary location:
sudo dd if=${HOME}/usb-backups/YYYYMMDD-HHMMSS/usb-image.img.gz of=/dev/loop0 \
    bs=4M status=progress
```
...
find ${HOME}/usb-backups/ -type d -mtime +30 -exec rm -rf {} \;

# Keep only last 5 backups:
ls -dt ${HOME}/usb-backups/*/ | tail -n +6 | xargs rm -rf

# Compress old backups:
find ${HOME}/usb-backups/ -name "*.img" -mtime +7 -exec gzip {} \;
```
...
sudo dd if=${HOME}/usb-backups/latest/usb-image.img.gz of=/dev/sdX bs=4M status=progress

# Or create fresh USB:
python3 scripts/setup.py --device /dev/sdX --iso ${HOME}/Downloads/ubuntu-24.04.4-desktop-amd64.iso
# Then restore persistence:
sudo dd if=${HOME}/usb-backups/latest/persistence.dat of=/dev/sdX5 bs=4M status=progress
```
...
# Then restore data:
sudo dd if=${HOME}/usb-backups/latest/persistence.dat of=/dev/sdX2 bs=4M status=progress
sudo cp -r ${HOME}/usb-backups/latest/ventoy-config/* /media/ventoy-usb/ventoy/
```
...
```bash
# Encrypt backups:
gpg -c ${HOME}/usb-backups/latest/usb-image.img.gz

# Decrypt for restore:
gpg -d ${HOME}/usb-backups/latest/usb-image.img.gz.gpg | \
