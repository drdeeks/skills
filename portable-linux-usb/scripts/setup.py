...
Usage:
    python3 setup.py --device /dev/sdX --iso ${HOME}/Downloads/ubuntu-24.04.4-desktop-amd64.iso
    python3 setup.py --device /dev/sdX --iso ${HOME}/Downloads/ubuntu-24.04.4-desktop-amd64.iso --persistence 20G
    python3 setup.py --device /dev/sdX --iso ${HOME}/Downloads/ubuntu-24.04.4-desktop-amd64.iso --agents alpha,beta,gamma
    python3 setup.py --device /dev/sdX --iso ${HOME}/Downloads/ubuntu-24.04.4-desktop-amd64.iso --global-deps 10G
"""
...
def detect_ventoy():
    ...
    ventoy_paths = [
        "/opt/ventoy",
        "/tmp/ventoy-1.1.12",
        os.path.expanduser("${HOME}/ventoy")
    ]
...
def backup_existing(device):
    ...
    if confirm_action("Backup existing USB data before proceeding?"):
        backup_path = f"${HOME}/usb-backup-{datetime.now().strftime('%Y%m%d-%H%M%S')}.img"
...
def mount_usb(device, mount_point="/media/ventoy-usb"):
...
alias usb-backup='sudo dd if=/dev/sdX of=${HOME}/usb-backup-$(date +%Y%m%d).img bs=4M status=progress'
"""
    
    print("Add these aliases to ${HOME}/.bashrc:")
    print(alias_script)
...
def main():
    ...
    parser.add_argument(
        "--mount-point",
        default="/media/ventoy-usb",
        help="Mount point for USB (default: /media/ventoy-usb)"
    )
