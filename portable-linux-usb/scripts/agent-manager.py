...
Usage:
    ...
    python3 agent-manager.py backup --name alpha --dest ${HOME}/backups/
    python3 agent-manager.py restore --name alpha --src ${HOME}/backups/alpha-backup.tar.gz
...
def backup_agent(args):
    ...
    backup_dir = args.dest or os.path.expanduser("${HOME}/agent-backups")
...
def delete_agent(args):
    ...
    backup_dir = os.path.expanduser("${HOME}/agent-deletion-backups")
    os.makedirs(backup_dir, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
