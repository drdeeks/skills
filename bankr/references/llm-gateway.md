...
3. Set default model in `${OPENCLAW_CONFIG_DIR}/openclaw.json`:
   ```json
...
6. Set default model in `${OPENCLAW_CONFIG_DIR}/openclaw.json` (see above)
...
```bash
# Write config to `${OPENCLAW_CONFIG_DIR}/openclaw.json`
...
```bash
# Auto-install Bankr provider into `${OPENCODE_CONFIG_DIR}/opencode.json`
