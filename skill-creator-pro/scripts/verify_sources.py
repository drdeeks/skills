...
def main():
    ...
    if args.report:
        report = generate_source_documentation(results[0] if len(results) == 1 else {'summary': {'total_urls': 0}})
    ...
    if args.generate_docs:
        for result in results:
            ...
            docs_path = target_path / 'SOURCES.md' if (target_path / 'SKILL.md').exists() else Path(args.target) / result['skill'] / 'SOURCES.md'
