...
class SkillPackager:
    ...
    def clean_nested_skill_files(self, skill_dir: Path) -> List[str]:
        """Remove any .skill files inside the skill directory (prevents nesting)."""
        removed = []
        for f in skill_dir.rglob("*.skill"):
            if f.is_file():
                removed.append(str(f.relative_to(skill_dir)))
                f.unlink()
        return removed
    ...
    def package_skill(self, skill_name: str, overwrite: bool = True, bump: bool = True) -> Dict:
        ...
        # Clean nested .skill files first
        removed = self.clean_nested_skill_files(skill_dir)
...
def main():
    ...
    if args.json:
        ...
        for r in details.get("packaged", []):
            ...
            cleaned = f" [cleaned {len(r.get('nested_cleaned', []))} nested .skill]" if r.get("nested_cleaned") else ""
            print(f"  ✓ {r['skill']}: v{r.get('version', '?')} | {r['files']} files, {r['size_kb']} KB{bump_info}{cleaned}")
