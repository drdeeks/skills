...
OpenClaw gateway can spawn hermes MCP brain locally. Just add `mcp.servers.hermes-brain` to the HOST `${OPENCLAW_DIR:-~/.openclaw}/openclaw.json`:
  ...
  "mcp": {
    "servers": {
      "hermes-brain": {
        ...
        "env": {
          "AGENT_ID": "hermes",
          "HERMES_HOME": "${HOME}/.openclaw/agents/hermes"
        }
      }
    }
  }
...
|--------|---------|-------|
| `scripts/main.py` | openclaw-hermes-mcp-integration script | Run with python3 |
