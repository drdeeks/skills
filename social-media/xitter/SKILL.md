---
description: Interact with X/Twitter via the x-cli terminal client using official
  X API credentials. Use for posting, reading timelines, searching tweets, liking,
  retweeting, bookmarks, mentions, and user lookups.
license: MIT
metadata:
  hermes:
...
If you already keep your X credentials in `${HERMES_DIR:-~/.hermes}/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf ${HERMES_DIR:-~/.hermes}/.env ~/.config/x-cli/.env
```
...
- **Credential drift**: if you rotate tokens in `${HERMES_DIR:-~/.hermes}/.env`, make sure `~/.config/x-cli/.env` still points at the current file.
