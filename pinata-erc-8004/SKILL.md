...
- Native Token: MON
- Identity Registry: `0x8004A169FB4a3325136EB29fA0ceB6D2e539a432` (SAME as Base/Ethereum — the universal ERC-8004 registry)
- **Note:** The address `0x8004BAa17C55a88189AE136b182e5fdA19dE9b63` found in some Monad docs is the **Reputation Registry**, NOT the Identity Registry. Do NOT use it for agent registration.
...
### ABI Formatting for setAgentURI
Viem requires the ABI as a proper object array, NOT a string. The following will fail:
```js
// WRONG - string format
abi: ['function setAgentURI(uint256 agentId, string newURI) external']
```
Use this instead:
```js
// CORRECT - object array
abi: [{ inputs: [{name:'agentId',type:'uint256'},{name:'newURI',type:'string'}], name: 'setAgentURI', outputs: [], stateMutability: 'nonpayable', type: 'function' }]
```

### ERC-8021 Builder Code Data Suffix on Monad
The `dataSuffix` from ERC-8021 builder code attribution (via `ox/erc8021`) causes `register()` to revert on Monad. This is likely because the Monad contract validates calldata length. Register WITHOUT the data suffix on Monad, then add attribution to subsequent transactions if needed.

### Contract Address Confusion
There are TWO addresses in the ERC-8004 ecosystem:
- **Identity Registry** (`0x8004A169FB4a3325136EB29fA0ceB6D2e539a432`) — Use this for agent registration (register, setAgentURI, transferFrom)
- **Reputation Registry** (`0x8004BAa17C55a88189AE136b182e5fdA19dE9b63`) — This is for reputation feedback, NOT for agent registration. On Monad it's a 262-byte stub.

### Private Key Format
Viem's `privateKeyToAccount()` requires the key with `0x` prefix. Always normalize:
```js
const pk = '0x' + rawKey.replace(/^0x/, '');
```

### Ownership Transfer Blocks URI Updates
After `transferFrom`, only the NEW owner can call `setAgentURI`. Plan your workflow: register → setURI → transfer.

### Multi-Chain Registration Workflow
1. Register on each chain separately (each gets a unique tokenId)
2. Upload agent card to IPFS with ALL registrations listed
3. Set on-chain URI on each chain to point to the same IPFS card
4. Transfer ownership on each chain
...
- If you get "invalid private key, expected hex or 32 bytes, got string", prepend `0x`:
  ```js
  const pk = '0x' + rawKey.replace(/^0x/, '');
  ```
...
### setAgentURI Fails with "ERC721NonexistentToken" or "Not authorized"
**Common causes:**
1. **Wrong ABI format** — viem requires the ABI as an object array, NOT a string. Use:
   ```js
   const ABI = [{
     inputs: [
   ...
   ```
   NOT: `[{ type: 'function', name: 'setAgentURI', inputs: [...], ... }]` with different key ordering, and definitely NOT a raw string like `'function setAgentURI(uint256 agentId, string newURI) external'`.

2. **"Not authorized" after transfer** — Only the current NFT owner can call `setAgentURI`. After transferring via `transferFrom`, the new owner must call it. You cannot update the URI from the previous owner's wallet.

3. **Token truly doesn't exist** — Verify with `ownerOf(tokenId)` first. If it returns an address, the token exists.
...
const REGISTRY = '0x8004A169FB4a3325136EB29fA0ceB6D2e539a432'; // Base/Ethereum mainnet
const ABI = [{
  inputs: [
    { name: 'agentId', type: 'uint256' },
    { name: 'newURI', type: 'string' }
...
async function main() {
  const pk = '0x' + fs.readFileSync('.secrets/.wallet', 'utf8')
  ...
  const regReceipt = await pc.waitForTransactionReceipt({ hash: regHash });
  ...
  // 2. Set Agent URI (use object ABI format!)
  const uriHash = await wc.writeContract({
    address: REGISTRY,
    abi: ABI,
    functionName: 'setAgentURI',
    args: [tokenId, 'ipfs://YOUR_AGENT_CARD_CID']
  });
  const uriReceipt = await pc.waitForTransactionReceipt({ hash: uriHash });
  console.log('URI set:', uriReceipt.status);
}
...
After `transferFrom`, only the new owner can call `setAgentURI`. If you need to update the URI after transfer, run the `setAgentURI` call from the new owner's wallet with their private key.

## Supported Networks

| Network | Chain ID | Registry Address | RPC |
|---------|----------|------------------|-----|
| Base Mainnet | 8453 | `0x8004A169FB4a3325136EB29fA0ceB6D2e539a432` | `https://mainnet.base.org` |
...
| Monad Testnet | 10143 | `0x8004BAa17C55a88189AE136b182e5fdA19dE9b63` | `https://testnet-rpc.monad.xyz` |
...
|--------|---------|-------|
| `scripts/main.py` | pinata-erc-8004 script | Run with python3 |
...
- Pinata Uploads: https://uploads.pinata.cloud
