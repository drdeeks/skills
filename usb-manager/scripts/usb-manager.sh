...
ventoy_setup() {
    ...
    print_info "Installing Ventoy to $device..."
...
mount_ventoy_persistence() {
    ...
    local mount_point="/mnt/ventoy-persistence"
    mkdir -p "$mount_point"
...
ventoy_persistence_copy() {
    ...
    if [[ -z "$mount_point" ]]; then
        ...
        sudo mkdir -p /mnt/ventoy-temp
        sudo mount "${device}1" /mnt/ventoy-temp
        mount_point="/mnt/ventoy-temp"
    fi
    ...
    local persist_mount="/mnt/ventoy-persistence"
    ...
    if [[ "$mount_point" == "/mnt/ventoy-temp" ]]; then
...
mount_disk_image() {
    ...
    local mount_point="/mnt/disk-image"
    mkdir -p "$mount_point"
...
manage_disk_images() {
    ...
    case "$choice" in
        ...
        3)
            ...
            ;;
        4)
            unmount_disk_image "/mnt/disk-image"
            ;;
        5)
        ...
        8)
            read -p "$(echo -e "${YELLOW}Source path to copy: ${NC}")" src_path
            if [[ -e "$src_path" ]]; then
                print_info "Copying $src_path to /mnt/disk-image..."
                sudo cp -a "$src_path" /mnt/disk-image/
                print_success "Copy complete"
            else
        ...
        9)
            read -p "$(echo -e "${YELLOW}Destination path: ${NC}")" dst_path
            mkdir -p "$dst_path"
            print_info "Copying from /mnt/disk-image to $dst_path..."
            sudo cp -a /mnt/disk-image/* "$dst_path/"
            print_success "Copy complete"
            ;;
        *)
            print_error "Invalid choice"
...
main() {
    ...
    case "${1:-}" in
        ...
        --image-unmount) unmount_disk_image "/mnt/disk-image"; exit 0 ;;
