...
```
${HERMES_DIR:-~/.hermes}/config.yaml       Main configuration
${HERMES_DIR:-~/.hermes}/.env              API keys and secrets
${HERMES_DIR:-~/.hermes}/skills/           Installed skills
${HERMES_DIR:-~/.hermes}/sessions/         Session transcripts
${HERMES_DIR:-~/.hermes}/logs/             Gateway and error logs
${HERMES_DIR:-~/.hermes}/auth.json         OAuth tokens and credential pools
${HERMES_DIR:-~/.hermes}/hermes-agent/     Source code (if git-installed)
```

Profiles use `${HERMES_DIR:-~/.hermes}/profiles/<name>/` with the same layout.
...
Check logs first:
```bash
grep -i "failed to send\|error" ${HERMES_DIR:-~/.hermes}/logs/gateway.log | tail -20
```
...
| Gateway logs | `${HERMES_DIR:-~/.hermes}/logs/gateway.log` |
| Session files | `${HERMES_DIR:-~/.hermes}/sessions/` or `hermes sessions browse` |
| Source code | `${HERMES_DIR:-~/.hermes}/hermes-agent/` |
...
|--------|---------|-------|
| `scripts/main.py` | hermes-agent script | Run with python3 |
...
Config: `${HERMES_DIR:-~/.hermes}/config.yaml` (settings), `${HERMES_DIR:-~/.hermes}/.env` (API keys).
...
- Tests auto-redirect `HERMES_HOME` to temp dirs — never touch real `${HERMES_DIR:-~/.hermes}/`
