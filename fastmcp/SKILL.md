...
```bash
python ${HERMES_DIR:-~/.hermes}/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ${HERMES_DIR:-~/.hermes}/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
- configure the server in `${HERMES_DIR:-~/.hermes}/config.yaml` using the `native-mcp` skill, or
...
The server-building part may be correct while the Hermes config is not. Load the `native-mcp` skill and configure the server in `${HERMES_DIR:-~/.hermes}/config.yaml`, then restart Hermes.
...
|--------|---------|-------|
| `scripts/scaffold_fastmcp.py` | fastmcp script | Run with python3 |

## Key References

- **Skill**: [references/SKILL.md](references/SKILL.md)
- **Fastmcp Cli**: [references/fastmcp-cli.md](references/fastmcp-cli.md)
## References
