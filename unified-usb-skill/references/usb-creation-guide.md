...
sudo dd if=/dev/sdX5 of=~/persistence-backup.dat bs=4M status=progress

# Restore persistence:
sudo dd if=~/persistence-backup.dat of=/dev/sdX5 bs=4M status=progress
```
...
```bash
# Copy to USB:
sudo cp ~/Downloads/*.iso /media/ventoy-usb/
